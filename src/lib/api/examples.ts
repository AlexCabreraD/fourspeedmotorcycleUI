// lib/api/examples.ts
// Usage examples for the WPS API Client

import { createWPSClient, WPSApiClient, ImageUtils } from './wps-client';
...
export async function exampleAdvancedFiltering() {
    try {
        ...
        console.log('Recent items:', recentItems.data.length);
...
export async function exampleCategoryBrowsing() {
    try {
        ...
        console.log('Category products:', categoryProducts.data);
...
export async function exampleInventoryCheck() {
    try {
        ...
        console.log('Item with inventory:', itemResponse.data);
        ...
        if (inventoryResponse.data.length > 0) {
            ...
            console.log(`Total inventory: ${inventory.total}`);
...
export async function examplePagination() {
    try {
        ...
        do {
            ...
            console.log(`Page ${pageCount + 1}:`, response.data.length, 'items');
        ...
        const allBrands = await wpsClient.getAllPages<any>('brands', {
            'page[size]': 50
        }, 5); // Max 5 pages

        console.log('All brands loaded:', allBrands.length);
    ...
    try {
        ...
        if (error instanceof Error) {
            if (error.name === 'WPSNotFoundError') {
                console.log('Product not found - expected behavior');
...
export class WPSHelpers {
    ...
    static getInventoryStatus(inventory: any): 'in_stock' | 'low_stock' | 'out_of_stock' {
        if (!inventory || inventory.total === 0) return 'out_of_stock';
        if (inventory.total < 5) return 'low_stock';
        return 'in_stock';
    }

    static getInventoryDisplay(inventory: any): string {
        if (!inventory) return 'Stock unknown';
        if (inventory.total === 0) return 'Out of stock';
        if (inventory.total >= 25) return 'In stock';
