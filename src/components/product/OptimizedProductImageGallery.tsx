...
const ProductBadges = memo(
  ({ savingsAmount, status }: { savingsAmount?: number | null; status?: string }) => {
    if (!savingsAmount && status !== 'STK') {
      return null
    }
      ...
      <div className='relative group'>
        ...
        <div className='card overflow-hidden transition-all duration-200 group-hover:shadow-card-hover'>
          ...
          >
            ...
            {/* Product badges */}
            <ProductBadges savingsAmount={savingsAmount} status={selectedItem?.status} />
          </div>
        </div>
