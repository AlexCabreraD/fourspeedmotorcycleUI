...
function SearchFilter({
  ...
  value,
  onChange,
}: Omit<FilterComponentProps, 'isExpanded' | 'onToggleExpanded'>) {
  return (
    <div className='mb-6'>
      <label className='block text-sm font-medium text-steel-900 mb-2'>{filter.label}</label>
...
export default function CustomCategoryFilters({
    ...
    const loadMetadata = async () => {
      ...
      try {
        const response = await fetch(
          `/api/custom-categories/${categorySlug}/metadata?include=price_range`
        )
  ...
  useEffect(() => {
    const defaultExpanded = new Set(filters.filter((f) => f.defaultExpanded).map((f) => f.key))
