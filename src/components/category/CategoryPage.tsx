...
export default function CategoryPage({ slug, searchParams }: CategoryPageProps) {
  ...
  }, [extractedFilters, sortBy, selectedBrands, priceRange, newArrivalsFilter, recentlyUpdatedFilter, weightRange, dimensionFilters])
      ...
      if (cachedResult && isCacheValid(cachedResult.timestamp)) {
        ...
        if (reset || isInitialLoad) {
          ...
          setProducts(prev => {
            ...
            const deduplicated = combined.filter((product: WPSItem, index: number, self: WPSItem[]) => 
  ...
  }, [category, sortBy, extractedFilters, currentPage, loading, fetchProducts, isSearchActive, selectedBrands, priceRange, newArrivalsFilter, recentlyUpdatedFilter, weightRange, dimensionFilters])
      ...
      searchTimeoutRef.current = setTimeout(() => {
        ...
        fetch(`${apiEndpoint}?${filterParams}&${searchParam}=${encodeURIComponent(searchTerm)}`)
          ...
          .then(data => {
            if (data.success && data.data) {
              // Filter out duplicates by ID
              const searchItems = data.data.filter((product: WPSItem, index: number, self: WPSItem[]) => 
                index === self.findIndex(p => p.id === product.id)
              )
  ...
  }, [selectedBrands, priceRange, newArrivalsFilter, recentlyUpdatedFilter, weightRange, dimensionFilters, isSearchActive, searchTerm, searchType, buildFilterParams, category])
  ...
  const performSearch = useCallback(async (query: string, type: 'name' | 'sku', cursor?: string) => {
    ...
    try {
      ...
      const apiEndpoint = `/api/custom-categories/${category!.slug}/items`
      const response = await fetch(`${apiEndpoint}?${filterParams}&${searchParam}=${encodeURIComponent(query)}${cursorParam}`)
      const data = await response.json()
            ...
            <div className="mt-2">
              ...
              {isSearchActive && !searching && (
                <p className="text-sm text-steel-600">
                  ...
                  <button onClick={clearSearch} className="text-primary-600 hover:text-primary-700 ml-1">
                    Clear search
                  </button>
                </p>
              )}
          ...
          <div className="flex items-center justify-between mt-6">
            <p className="text-steel-600">
              {isSearchActive ? (
                ...
                ) : searching ? 'Searching...' : `No results found for "${searchTerm}"`
              ) : (
                totalLoaded > 0 ? (
                  hasMore ? 
          ...
          <div className="md:col-span-3">
            {(productsLoading && !isSearchActive) || searching ? (
              ...
              <>
                ...
                {/* Show More Button */}
                {!productsLoading && !searching && (isSearchActive ? searchResults : products).length > 0 && (
