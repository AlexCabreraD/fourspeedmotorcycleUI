...
export default function OptimizedCategoryPage({ slug, searchParams }: OptimizedCategoryPageProps) {
  ...
  const buildFilterParams = useCallback(() => {
    ...
    if (recentlyUpdatedFilter) {
      const now = new Date()
      const daysAgo = new Date(now.getTime() - (parseInt(recentlyUpdatedFilter) * 24 * 60 * 60 * 1000))
      params.set('updated_after', daysAgo.toISOString().split('T')[0])
    }
    
    // Weight range filters
    if (weightRange.min) params.set('min_weight', weightRange.min)
    if (weightRange.max) params.set('max_weight', weightRange.max)
  ...
  }, [currentFilters, sortBy, selectedBrands, priceRange, newArrivalsFilter, recentlyUpdatedFilter, weightRange, dimensionFilters])
      ...
      if (cachedResult && isCacheValid(cachedResult.timestamp)) {
        if (reset || isInitialLoad) {
          ...
          setProducts(prev => {
            ...
            const deduplicated = combined.filter((product, index, self) => 
  ...
  }, [category, currentFilters, sortBy, loading, fetchProducts, isSearchActive, selectedBrands, priceRange, newArrivalsFilter, recentlyUpdatedFilter, weightRange, dimensionFilters])
