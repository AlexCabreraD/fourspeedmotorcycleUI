...
export default function AdvancedCategoryFilters({
  ...
  const organizedBrands = useMemo(() => {
    ...
    const selectedRelevant = relevantBrands.filter(brand => selectedBrands.includes(brand))
    const selectedAdditional = additionalBrands.filter(brand => selectedBrands.includes(brand))
    const unselectedRelevant = relevantBrands.filter(brand => !selectedBrands.includes(brand))
    const unselectedAdditional = additionalBrands.filter(brand => !selectedBrands.includes(brand))
    ...
    return {
      ...
      showingAll: showAllBrands || brandSearchTerm.length > 0,
