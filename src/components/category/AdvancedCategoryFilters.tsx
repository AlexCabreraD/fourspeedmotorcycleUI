...
import {
  getOtherBrands,
  getPrioritizedBrandsForProductTypes,
} from '@/lib/constants/brand-product-mapping'
...
export default function AdvancedCategoryFilters({
  ...
  const [expandedSections, setExpandedSections] = useState<Set<string>>(
    new Set(['category-types', 'brands', 'price'])
  )
  ...
  const organizedBrands = useMemo(() => {
    ...
    if (brandSearchTerm && brandSearchTerm.length > 0) {
      ...
      relevantBrands = prioritizedBrands.filter(
        (brand) => brand && brand.toLowerCase().includes(searchLower)
      )
      additionalBrands = otherBrands.filter(
        (brand) => brand && brand.toLowerCase().includes(searchLower)
      )
    } else {
    ...
    const selectedAdditional = additionalBrands.filter((brand) =>
      safeSelectedBrands.includes(brand)
    )
    const unselectedRelevant = relevantBrands.filter((brand) => !safeSelectedBrands.includes(brand))
    const unselectedAdditional = additionalBrands.filter(
      (brand) => !safeSelectedBrands.includes(brand)
    )
      ...
      <div className={`space-y-6 ${isLoading ? 'opacity-50' : ''}`}>
        ...
        <div>
          ...
          >
            ...
            <ChevronDown
              className={`h-4 w-4 transform transition-transform ${expandedSections.has('brands') ? 'rotate-180' : ''}`}
            />
          </button>
              ...
              <div className='max-h-64 overflow-y-auto p-3'>
                <div className='space-y-2'>
                  ...
                  {organizedBrands.selected.map((brand) => (
                    <label
                      key={brand}
                      className='flex items-center cursor-pointer hover:bg-steel-50 p-1 rounded bg-primary-50'
                    >
                      <input
                        type='checkbox'
                        checked={true}
                      ...
                      />
                      <span className='text-sm text-primary-700 font-medium'>{brand}</span>
                    </label>
                  ))}

                  {/* Separator if we have selected brands and other brands */}
                  {organizedBrands.selected.length > 0 &&
                  ...
                  {organizedBrands.relevantUnselected.length > 0 && (
                    <>
                      {!brandSearchTerm && organizedBrands.selected.length === 0 && (
                        <div className='mb-2'>
                          <p className='text-xs text-steel-600 font-medium mb-1'>
                            Popular for {category?.name}:
                          </p>
                        </div>
                      )}
                      {organizedBrands.relevantUnselected.map((brand) => (
                        <label
                          key={brand}
                          className='flex items-center cursor-pointer hover:bg-steel-50 p-1 rounded'
                        >
                          <input
                            type='checkbox'
                            checked={false}
                            ...
                            className='form-checkbox text-primary-600 rounded mr-3'
                          />
                          <span className='text-sm text-steel-700'>{brand}</span>
                        </label>
                      ))}
                    </>
                  )}
                      ...
                      {organizedBrands.additionalUnselected.map((brand) => (
                        ...
                        >
                          <input
                            ...
                            className='form-checkbox text-primary-600 rounded mr-3'
                          />
                          <span className='text-sm text-steel-600'>{brand}</span>
                        </label>
                      ))}
                    </>
                  )}
                  ...
                  {!organizedBrands.showingAll && organizedBrands.hasAdditionalBrands && (
                    <div className='border-t border-steel-200 mt-3 pt-3'>
                      ...
                      >
                        ...
                        {
                          getOtherBrands(
                            getPrioritizedBrandsForProductTypes(category?.productTypeFilters || [])
                          ).length
                        }{' '}
                        more)
                      </button>
        ...
        <div>
          ...
          >
            Price Range (USD)
            <ChevronDown
              className={`h-4 w-4 transform transition-transform ${expandedSections.has('price') ? 'rotate-180' : ''}`}
            />
          </button>
        ...
        <div>
          ...
          >
            New Arrivals
            <ChevronDown
              className={`h-4 w-4 transform transition-transform ${expandedSections.has('arrivals') ? 'rotate-180' : ''}`}
            />
          </button>
        ...
        <div>
          ...
          >
            Recently Updated
            <ChevronDown
              className={`h-4 w-4 transform transition-transform ${expandedSections.has('updated') ? 'rotate-180' : ''}`}
            />
          </button>
        ...
        <div>
          ...
          >
            Weight Range (lbs)
            <ChevronDown
              className={`h-4 w-4 transform transition-transform ${expandedSections.has('weight') ? 'rotate-180' : ''}`}
            />
          </button>
        ...
        <div>
          ...
          >
            Dimensions (inches)
            <ChevronDown
              className={`h-4 w-4 transform transition-transform ${expandedSections.has('dimensions') ? 'rotate-180' : ''}`}
            />
          </button>
        ...
        {category && (
          <div className='pt-4 border-t border-steel-200'>
            ...
            >
              ...
              <ChevronDown
                className={`h-3 w-3 transform transition-transform ${expandedSections.has('category-types') ? 'rotate-180' : ''}`}
              />
            </button>
              ...
              <div className='bg-steel-50 rounded-md p-2'>
                <div className='flex flex-wrap gap-1'>
                  {category.productTypeFilters.map((type) => (
                    <span
                      key={type}
                      className='inline-flex items-center px-2 py-0.5 bg-steel-200 text-steel-700 text-xs rounded'
                    >
                      {type}
                    </span>
                  ))}
