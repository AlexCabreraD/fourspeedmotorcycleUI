    ...
    <>
      ...
      <div className='fixed right-0 top-0 h-full w-full max-w-md bg-white shadow-xl z-50 flex flex-col'>
        ...
        <div className='flex items-center justify-between p-4 border-b border-steel-200'>
          <h2 className='text-lg font-semibold text-steel-900'>Shopping Cart ({totalItems})</h2>
          <button onClick={closeCart} className='p-2 text-steel-400 hover:text-steel-600'>
            <X className='h-5 w-5' />
          </button>
        </div>
          ...
          {isLoading ? (
            ...
            <div className='p-4 space-y-4'>
              {items.map((item) => (
                <div
                  key={item.id}
                  className='flex items-start space-x-3 bg-steel-50 rounded-lg p-3'
                >
                  {/* Product Image */}
                  <img
                  ...
                  <div className='flex-1 min-w-0'>
                    <h4 className='text-sm font-medium text-steel-900 line-clamp-2'>{item.name}</h4>
        ...
        {items.length > 0 && (
          <div className='border-t border-steel-200 p-4 space-y-4'>
            ...
            <div className='space-y-2'>
              ...
              {!selectedShippingRate && (
                <p className='text-xs text-steel-500'>Calculated at checkout</p>
              )}
              {selectedShippingRate && shippingTotal > 0 && (
                <p className='text-xs text-steel-500'>Free shipping on orders over $99</p>
              )}
