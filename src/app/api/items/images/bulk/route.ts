  ...
  try {
    ...
    const itemImages = response.data.map(item => {
      ...
      const allImageUrls = hasImages 
        ? ImageUtils.getItemImageUrls(item, '500_max')
        : []
...
export async function GET(request: NextRequest) {
  try {
    ...
    const item_ids = itemIdsParam.split(',').map(id => parseInt(id.trim())).filter(id => !isNaN(id))
    ...
    const errorMessage = error instanceof Error ? error.message : 'Failed to process bulk image request'
