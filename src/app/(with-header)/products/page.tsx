...
import { ArrowUp, Filter, Grid, Heart, List, Package, Search, X } from 'lucide-react'
...
function ProductsContent() {
  ...
  const fetchProducts = useCallback(
    async (cursor?: string, reset: boolean = false) => {
      ...
      // Set loading states
      if (isInitialLoad && !loadingFilters) {
        setLoading(true)
    ...
    async (query: string, type: 'name' | 'sku', cursor?: string) => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/products?${filterParams}&${searchParam}=${encodeURIComponent(query)}${cursorParam}`
        )
        const data = await response.json()
  ...
  const loadMoreProducts = useCallback(async () => {
    ...
    if (isSearchActive) {
      ...
      try {
        ...
        const searchParam = searchType === 'name' ? 'search' : 'sku'
        const response = await fetch(
          `/api/products?${filterParams}&${searchParam}=${encodeURIComponent(searchTerm)}&cursor=${nextCursor}`
        )
        const data = await response.json()
  ...
  const activeFiltersCount = [
    ...
    dimensionFilters.length.min ||
      dimensionFilters.length.max ||
      dimensionFilters.width.min ||
      dimensionFilters.width.max ||
      dimensionFilters.height.min ||
      dimensionFilters.height.max,
  ...
  const handleItemTypeChange = (itemType: string) => {
    setSelectedItemTypes((prev) =>
      prev.includes(itemType) ? prev.filter((t) => t !== itemType) : [...prev, itemType]
    )
  }
  ...
  const filteredBrands = useMemo(() => {
    const brands = !brandSearchTerm
      ? ALL_BRANDS
      : ALL_BRANDS.filter((brand) => brand.toLowerCase().includes(brandSearchTerm.toLowerCase()))
  ...
  return (
    <div className='min-h-screen bg-white'>
      <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16'>
        {/* Page Header */}
        <div className='mb-8'>
          <h1 className='text-4xl font-display font-bold text-steel-900 mb-4'>All Products</h1>
              ...
              >
                <div
                  className={`w-4 h-4 rounded border-2 flex items-center justify-center transition-all ${
                    showInStockOnly ? 'bg-green-500 border-green-500' : 'border-steel-400'
                  }`}
                >
                  {showInStockOnly && (
            ...
            <div className='mt-3'>
              {searchTerm.length > 0 && searchTerm.length < 3 && (
                <div className='text-sm text-steel-500'>
                  ...
                  {3 - searchTerm.length !== 1 ? 's' : ''} to search
                </div>
              )}
              {searching && <div className='text-sm text-primary-600'>Searching...</div>}
              ...
              {isSearchActive && searchResults.length === 0 && !searching && (
                <div className='text-sm text-steel-500'>No results found for "{searchTerm}"</div>
              )}
            </div>
          </div>
                    ...
                    <div className='max-h-40 overflow-y-auto'>
                      <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-1 p-2'>
                        {filteredBrands.map((brand) => (
                          <label
                            key={brand}
                            className='flex items-center space-x-2 p-2 hover:bg-steel-50 rounded cursor-pointer'
                          >
                            <input
                              type='checkbox'
          ...
          <div className='flex flex-col lg:flex-row gap-4 justify-between items-start lg:items-center mb-6'>
            ...
            <div className='flex items-center gap-4'>
              ...
              {hasMore && (
                <p className='text-steel-500 text-sm'>Load more to see additional results</p>
              )}
            </div>
            ...
            <div className='flex flex-wrap gap-2 mb-6'>
              ...
              {recentlyUpdatedFilter && (
                <span className='inline-flex items-center gap-1 bg-cyan-100 text-cyan-800 px-3 py-1 rounded-full text-sm'>
                  ...
                  <button
                    onClick={() => setRecentlyUpdatedFilter('')}
                    className='hover:bg-cyan-200 rounded-full p-1'
                  >
                    <X className='h-3 w-3' />
                  </button>
                </span>
              ...
              {(weightRange.min || weightRange.max) && (
                <span className='inline-flex items-center gap-1 bg-orange-100 text-orange-800 px-3 py-1 rounded-full text-sm'>
                  ...
                  <button
                    onClick={() => setWeightRange({ min: '', max: '' })}
        ...
        {loadingFilters ? (
          ...
          >
            {[...Array(itemsPerPage)].map((_, i) => (
              ...
              >
                ...
                <div
                  className={`relative bg-steel-50 overflow-hidden ${
                ...
                <div className={`p-4 ${viewMode === 'list' ? 'flex-1' : ''}`}>
                  ...
                  {/* Product Name */}
                  <div className={`space-y-2 mb-2 ${viewMode === 'list' ? '' : 'h-10'}`}>
                  ...
                  {/* Add to Cart Button */}
                  <div
                    className={`h-10 bg-steel-200 rounded animate-pulse ${
                ...
                >
                  ...
                  >
                    {ImageUtils.hasImages(product) ? (
                      ...
                      <ImagePlaceholder message='Image coming soon' className='w-full h-full' />
                    )}
                  ...
                  <div className={`p-4 ${viewMode === 'list' ? 'flex-1' : ''}`}>
                    ...
                    {/* Product Name */}
                    <h3
                      className={`font-medium text-steel-900 mb-2 hover:text-primary-600 transition-colors ${
                    ...
                    {product.product_type && (
                      <p className='text-xs text-steel-500 mb-2'>{product.product_type}</p>
                    )}
                    ...
                    {/* SKU */}
                    <p className='text-xs text-steel-400 mb-3'>SKU: {product.sku}</p>
          ...
          <div className='text-center py-12'>
            ...
            <h3 className='text-lg font-medium text-steel-900 mb-2'>No products found</h3>
            <p className='text-steel-600 mb-6'>Try adjusting your search terms or filters.</p>
