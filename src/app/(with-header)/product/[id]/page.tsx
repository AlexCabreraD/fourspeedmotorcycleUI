...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_WPS_API_URL || 'http://localhost:3000'}/api/products/${id}`
    )
    ...
    if (data.success && data.data) {
      ...
      return {
        ...
        description:
          product.description ||
          `Buy ${product.name} at 4Speed Motorcycle. High-quality motorcycle parts and accessories.`,
        keywords: [
          product.name,
        ...
        openGraph: {
          title: product.name,
          description:
            product.description || `High-quality ${product.name} available at 4Speed Motorcycle`,
          type: 'product',
